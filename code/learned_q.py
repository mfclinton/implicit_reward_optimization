...
def Run_Gridworld_Implicit(T1, T2, T3, approximate, reuse_trajectories):
    ...
    for t1 in range(T1):
        ...
        for t2 in range(T2):
            ...
            delta = - (phi * in_rewards.view(-1,1)).sum(axis=0)
            # print(agent.model.linear1.weight.size(), delta.size())
            agent.model.zero_grad()
            agent.model.linear1.weight.grad = delta.detach().view(env.action_space.n, env.state_space.n)
        ...
        for t3 in range(T3):
            ...
            # (T) Debug
            discounted_real_rewards = (real_rewards * cumu_gammas).sum(axis=1)
            ...
            c += (phi * discounted_real_rewards.view(-1,1)).sum(dim=0)
            # b += cumu_phi * in_gammas * d_in_gamma * ( - log_probs)
            # b += Calculate_B()

            if (approximate):
                H += (phi*phi*torch.abs(in_rewards.view(-1,1))).sum(axis = 0)
                A += (phi * d_in_reward).sum(dim = 0)
